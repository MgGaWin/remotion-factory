...
- 暗色场景比例：20-35%
- 全片第 0 场景（第一个 Scene 组件）必须是暗色场景（含 className="dark-theme"）
- 连续简洁帧不超过 2 个
- 连续密集帧不超过 3 个
- 连续场景布局不重复
- 每章至少使用 2-3 种不同布局 ID
...
- 信息/科普解释模式：每个场景的观众问题满足质量标准——具体（不是「什么是 XX」这种宽泛问题）、前置（写在观众嘴里还说不出来、看完这屏能脱口回答的那句话的反面）、制造好奇（暗示答案不重要但反直觉）

**PASS：** 场景/段落数一致；暗色比例 20-35%；首场景暗色；无连续同布局；布局种类 >= 3；每章有标题场景；观众问题满足三条标准
**FAIL：** 场景/段落数不匹配；暗色比例超出范围；首场景非暗色；3+ 连续同布局；布局种类 < 3；章节缺标题场景；观众问题不符合质量标准
...
- 暗色场景必须有 className="dark-theme" 且 style 中显式设置 background: 'var(--c-bg)'
- 全片第 0 场景必须是暗色场景
- 每章至少使用 2-3 种不同布局 ID
- 全屏组件（CC/TS）前后场景必须为非全屏场景
- 所有内容 y < 930
...
**PASS：** 暗色比例 20-35%；首场景暗色；无连续同布局；布局种类 >= 3；暗色场景有 dark-theme + background；所有内容 y < 930；无 AI 味特征；Studio 无报错
**FAIL：** 暗色比例超出范围；首场景非暗色；3+ 连续同布局；布局种类 < 3；暗色场景缺 dark-theme 或 background；内容超出安全区；有 AI 味特征；Studio 报错
